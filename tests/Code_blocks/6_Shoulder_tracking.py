...
def trackPose(me, shoulderLength, cx, cy, w, h, pid, pError, pid_height, pError_height):
    error = cx-w//2
    ...
    if shoulderLength >shLenRange[0] and shoulderLength < shLenRange[1]:
        fb = 0
    elif shoulderLength >shLenRange[1]:
        fb = -20
    #elif shoulderLength < shLenRange[0] and shoulderLength!= 0:
    elif shoulderLength < shLenRange[0]:
        fb = 20
    else: fb = 0
...
while True:
    ...
    if len(lmList_pose)!=0:
        if len(lmList_pose[11])!=0 and len(lmList_pose[12])!=0:
            ...
            pError,pError_height = trackPose(me, shoulderLength, cx, cy, w, h, pid, pError, pid_height, pError_height)
    cTime = time.time()
    fps = 1/(cTime - pTime)
    pTime = cTime
